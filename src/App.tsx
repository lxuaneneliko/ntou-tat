...
function MailScreen({ studentId }: { studentId: string }) {
  ...
  const loadInbox = useCallback(async (activeCredentials: MailCredentials) => {
    setBusy(true)
    setError(null)
    try {
      const nextInbox = await mailApi.listMessages(activeCredentials)
      setInbox(nextInbox)
      setDetail(null)
      return true
    } catch (loadError) {
      if (mailErrorCode(loadError) === 'MAIL_AUTH_FAILED') {
  ...
  const login = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (remember) {
        await mailCredentialsStore.save(normalized)
      } else {
        await mailCredentialsStore.clear()
      }
  ...
  const openMessage = async (message: MailSummary) => {
    ...
    setError(null)
    try {
      setDetail(await mailApi.getMessage(credentials, message.uid))
    } catch (loadError) {
      setError(messageFromError(loadError))
    } finally {
    ...
    return (
      <section className="mail-screen mail-login-screen">
        <div className="mail-account-strip">
          ...
          <div>
            <strong>登入 Mail2000</strong>
            <span>信箱密碼與 AIS 密碼分開輸入</span>
          </div>
        </div>
        <form className="mail-login-form" onSubmit={(event) => void login(event)}>
          <label>
            <span>信箱帳號</span>
            <div className="mail-account-input">
              <input
                autoComplete="username"
                inputMode="text"
                value={account}
          ...
          <label className="mail-remember-row">
            <input
              type="checkbox"
              checked={remember}
              onChange={(event) => setRemember(event.target.checked)}
            />
            <span>加密儲存這組信箱登入</span>
          </label>
          {error ? (
          ...
          <button className="mail-login-button" type="submit" disabled={busy || !account.trim() || !password}>
            {busy ? <Loader2 className="spin" size={20} /> : <KeyRound size={20} />}
            <span>{busy ? '登入中' : '登入信箱'}</span>
          </button>
        </form>
        <div className="mail-privacy-row">
          <ShieldCheck size={18} />
          <span>信箱帳密獨立存放於本機安全區，不會傳到海大 TAT 伺服器</span>
        </div>
      </section>
    )
  }
    ...
    return (
      <section className="mail-screen mail-detail-screen">
        <div className="mail-detail-toolbar">
          <button type="button" aria-label="返回收件匣" onClick={() => setDetail(null)}>
            <ChevronLeft size={23} />
          </button>
          <span>信件內容</span>
        </div>
        <article className="mail-detail">
          ...
          <div className="mail-detail-sender">
            ...
            <time>{formatMailDate(detail.receivedAt)}</time>
          </div>
          ...
          {detail.attachments.length ? (
            <div className="mail-attachments">
              <strong>附件</strong>
              {detail.attachments.map((name, index) => <span key={`${index}-${name}`}>{name}</span>)}
            </div>
          ) : null}
        </article>
      </section>
    ...
    <section className="mail-screen">
      <div className="mail-account-strip">
        ...
        <div>
          ...
          <span>{inbox?.account ?? credentials.account}@mail.ntou.edu.tw</span>
        </div>
        <div className="mail-account-actions">
          <button type="button" aria-label="重新整理收件匣" disabled={busy} onClick={() => void loadInbox(credentials)}>
            <RefreshCw className={busy ? 'spin' : ''} size={20} />
          </button>
          <button type="button" aria-label="登出信箱" onClick={() => void logoutMail()}>
            <LogOut size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="mail-inbox-meta">
        <strong>收件匣</strong>
        <span>{inbox ? `${inbox.unread} 封未讀 · 共 ${inbox.total} 封` : '載入中'}</span>
      </div>
      {busy && !inbox ? (
        <div className="mail-loading"><Loader2 className="spin" size={28} /><span>正在讀取收件匣</span></div>
        ...
        <div className="mail-message-list">
          {inbox.messages.map((message) => (
            <button
              ...
              type="button"
              key={message.uid}
              disabled={detailBusyUid !== null}
              onClick={() => void openMessage(message)}
            >
              <span className="mail-unread-dot" aria-hidden="true" />
              <span className="mail-sender-avatar">{message.sender.slice(0, 1).toUpperCase()}</span>
              <span className="mail-message-copy">
                <strong>{message.sender}</strong>
                <span>{message.subject}</span>
              </span>
              <time>{detailBusyUid === message.uid ? <Loader2 className="spin" size={16} /> : formatMailDate(message.receivedAt)}</time>
            </button>
          ))}
        </div>
      ) : inbox ? (
        <div className="mail-empty"><PackageOpen size={36} /><span>收件匣目前沒有信件</span></div>
      ) : null}
    </section>
  )
}
