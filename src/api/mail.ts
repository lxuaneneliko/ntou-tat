...
export const mailApi = {
  ...
  async listMessages(credentials: MailCredentials, folder: string, offset = 0, limit = 30) {
    ...
    if (isMailMock) {
      ...
      return { account: normalizeMailAccount(credentials.account), folder, total: messages.length, unread: messages.filter((message) => message.unread).length, offset, nextOffset: offset + messages.length, hasMore: false, messages }
    }
    return NativeMail.listMessages({ ...normalizedCredentials(credentials), folder, offset, limit })
  },
