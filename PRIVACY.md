...
PWA 版本提供本機學號登入與海大 AIS 官方登入頁入口。學號只保存在目前瀏覽器，AIS
密碼則直接由海大官方網站處理，海大 TAT 不會要求、接收或保存。AIS 未開放跨網域
存取，因此瀏覽器仍無法安全重現 Android 原生 App 的 Cookie Session 流程或讀取個人資料。
...
- PWA 學號與自訂資料只保存在目前瀏覽器的本機儲存空間；清除網站資料會一併移除。
